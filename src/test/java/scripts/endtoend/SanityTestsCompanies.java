...
public class SanityTestsCompanies extends TestBase {
...
//		}
	...
	public void startTest() {
		...
		try {
			userCredentials = localExcelUtility.getCompleteSheetData(getWorkbookPath_AccountsForTesting(),
					"EndToEndCompanies", "CompaniesSanityTest");
	...
	@Test(priority = 2)
	public void AddCompanyusingHeaderButton() {
		userCredentials.companyName = userCredentials.companyHeader;
		pageObjectManager.setUserCredentials(userCredentials);
		try {
			headerAddCompany = new HeaderAddCompany(driver, wait, pageObjectManager, testResultsStatus);
			headerAddCompany.logicForHeaderAddCompany();
		} catch (Exception e) {
			testResultsStatus.setTestResult("fail");
			testResultsStatus.setTestResultComment("Error while creating a company " + e.getMessage());
			Assert.assertTrue(false);
	...
	@Test(priority = 3)
	public void AddCompanyusingBottomButton() {
		userCredentials.companyName = userCredentials.companyBottom;
		pageObjectManager.setUserCredentials(userCredentials);
		try {
			addSubCompanyBelowExistingCompany = new AddSubCompanyBelowExistingCompany(driver, wait, pageObjectManager,
					testResultsStatus);
			addSubCompanyBelowExistingCompany.logicForAddSubCompanyBelowExistingCompany();
		} catch (Exception e) {
			testResultsStatus.setTestResult("fail");
			testResultsStatus.setTestResultComment("Error while creating a company " + e.getMessage());
			Assert.assertTrue(false);
