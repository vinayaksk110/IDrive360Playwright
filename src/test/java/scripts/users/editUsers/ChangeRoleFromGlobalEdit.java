...
public class ChangeRoleFromGlobalEdit extends TestBase{
	UserCredentials userCredentials = null;
	PageObjectManager pageObjectManager = null;
	WebDriver driver = null;
	Wait<WebDriver> wait = null;

	public ChangeRoleFromGlobalEdit() {

	}

	public ChangeRoleFromGlobalEdit(PageObjectManager pageObjectManager, WebDriver driver, Wait<WebDriver> wait,
	...
	public void initializeTest(@Optional("yes") String excelFileStatus, @Optional("chrome") String browser,
		...
		testResultsStatus.setTestName(ChangeRoleFromGlobalEdit.class.getSimpleName());
	}
	...
	public void startTest() {
		...
		logicForChangeRoleFromGlobalEdit(pageObjectManager.getUserCredentials().userToBeEdited);

	}

	public TestResultsStatus logicForChangeRoleFromGlobalEdit(String userEmail) {
		...
		try {
			...
			// Move to the user to change the Role and perform view icon action
			pageObjectManager.getUsersPage().navigateToViewIcon(userEmail, testResultsStatus);
	

			// Click on Edit & Change the role
			try {
				...
				testResultsStatus.setTestResultComment(e.getMessage());
				log.error(e.getMessage());
				log.error("Error while unchecking the company admin checkbox");
				Assert.assertTrue(false);
			}

			// asserting un-check of Admin role
			try {
				Assert.assertEquals(pageObjectManager.getUsersPage().companyAdministratorRole.isSelected(), false);
			...
			// Checking checkbox - Backup administrator user Role & Save
			pageObjectManager.getUsersPage().checkRolesFetchedFromExcel(userCredentials.role);
			pageObjectManager.getUsersPage().btnSaveChanges.click();
