...
public class UpdateLastNameFromGlobalEdit extends TestBase {
	...
	public UpdateLastNameFromGlobalEdit() {

	}

	public UpdateLastNameFromGlobalEdit(PageObjectManager pageObjectManager, WebDriver driver, Wait<WebDriver> wait,
	...
	public void initializeTest(@Optional("yes") String excelFileStatus, @Optional("chrome") String browser,
		...
		testResultsStatus.setTestName(UpdateLastNameFromGlobalEdit.class.getSimpleName());
	}
	...
	public void startTest() {
		...
		logicForUpdateLastNameFromGlobalEdit(pageObjectManager.getUserCredentials().userToBeEdited);

	}

	public TestResultsStatus logicForUpdateLastNameFromGlobalEdit(String userEmail) {
		...
		try {
			...
			// navigate to user to be updated with last name
			try {
				pageObjectManager.getUsersPage().navigateToUser(userEmail);
				log.info("Found the user to be updated with last name");
			} catch (Exception e) {
			...
			// Move to the user to be updated with last name and perform view icon action
			pageObjectManager.getUsersPage().navigateToViewIcon(userEmail, testResultsStatus);

			// Click on Edit & Update last name
			try {
				pageObjectManager.getUsersPage().clickOnGlobalEditIcon(userEmail, testResultsStatus);
				pageObjectManager.getUsersPage().updateLastName(userCredentials.lastName);
			} catch (Exception e) {
				e.printStackTrace();
				testResultsStatus.setTestResult("Fail");
				testResultsStatus.setTestResultComment(e.getMessage());
				log.error(e.getMessage());
				log.error("Error while updating the last name");
				Assert.assertTrue(false);
			}
			// success message
