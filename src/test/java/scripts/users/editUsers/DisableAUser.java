...
public class DisableAUser extends TestBase {
	...
	public DisableAUser(PageObjectManager pageObjectManager, WebDriver driver, Wait<WebDriver> wait,
	...
	public void initializeTest(@Optional("yes") String excelFileStatus, @Optional("chrome") String browser,
		...
		testResultsStatus.setTestName(DisableAUser.class.getSimpleName());
	}
	...
	public void startTest() {
		...
		logicForDisableAUser(pageObjectManager.getUserCredentials().userToBeDisabled);
	...
	public TestResultsStatus logicForDisableAUser(String userEmail) {
		...
		try {
			...
			// Check user status
			try {
				log.info("User email considered for disabling is : " + userEmail);
				if (pageObjectManager.getUsersPage().VerifyUserStatus(userEmail).equals("Active")) {
					log.info("User status is Active hence continuing with Disable flow");
				} else {
			...
			// navigate to user to be disabled
			try {
				pageObjectManager.getUsersPage().navigateToUser(userEmail);
				log.info("Found the user to be disabled");
			} catch (Exception e) {
			...
			// Move to the user to be disabled and perform Disable action
			try {
				pageObjectManager.getUsersPage().navigateToDisableIcon(userEmail);
			} catch (Exception e) {
				e.printStackTrace();
				testResultsStatus.setTestResult("Fail");
				testResultsStatus.setTestResultComment(e.getMessage());
				log.error(e.getMessage());
				log.error("Error while navigating to the Disable icon");
				Assert.assertTrue(false);
			}

			// Disable user
			try {
				pageObjectManager.getUsersPage().disableUser(userEmail);
			} catch (Exception e) {
				e.printStackTrace();
				testResultsStatus.setTestResult("Fail");
				testResultsStatus.setTestResultComment(e.getMessage());
				log.error(e.getMessage());
				log.error("Error while disabling the user");
				Assert.assertTrue(false);
			}
			// success message
			String message = pageObjectManager.getUsersPage().waitForMessage();
			try {
				Assert.assertEquals(message, "The user " + userEmail + " has been disabled");
		...
		pageObjectManager.getActivityLogsPage().AssertActivityLogs(
				getMessage("activityLogs.Userdisabled").replace("{companyName}", userCredentials.companyName)
