...
public class AddUserFromCentreButton extends TestBase {
	UserCredentials userCredentials = null;
	PageObjectManager pageObjectManager = null;
	WebDriver driver = null;
	Wait<WebDriver> wait = null;

	public AddUserFromCentreButton() {

	}

	public AddUserFromCentreButton(PageObjectManager pageObjectManager, WebDriver driver, Wait<WebDriver> wait,
	...
	public void initializeTest(@Optional("yes") String excelFileStatus, @Optional("chrome") String browser,
		...
		testResultsStatus.setTestName(AddUserFromCentreButton.class.getSimpleName());
	}
	...
	public TestResultsStatus logicForAddUserInSubCompanies() {
		pageObjectManager.getUserCredentials().subUserEmail = dateNTime
				.emailCreationWithDate(pageObjectManager.getUserCredentials().subUserEmail);
		try {
			...
			pageObjectManager.getUsersPage().selectCompany(userCredentials.companyName, testResultsStatus);

			// Click Add user button from bottom of page
			pageObjectManager.getUsersPage().clickcenterAddUser(testResultsStatus);

			// Enter emailId in text field
			pageObjectManager.getUsersPage().enterEmailIds(filePath, testResultsStatus);
