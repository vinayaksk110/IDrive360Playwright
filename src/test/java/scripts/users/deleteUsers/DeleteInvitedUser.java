...
public class DeleteInvitedUser extends TestBase {
	...
	public DeleteInvitedUser(PageObjectManager pageObjectManager, WebDriver driver, Wait<WebDriver> wait,
	...
	public void initializeTest(@Optional("yes") String excelFileStatus, @Optional("chrome") String browser,
		...
		testResultsStatus.setTestName(DeleteInvitedUser.class.getSimpleName());
	}
	...
	public void startTest() {
		...
		logicForDeleteInvitedUser(pageObjectManager.getUserCredentials().userToBeDeleted);
	}

	public TestResultsStatus logicForDeleteInvitedUser(String userEmail) {
		...
		try {
			...
			try {
				...
				if (pageObjectManager.getUsersPage().VerifyUserStatus(userEmail).equals("Inactive")) {
					log.info("User status is Inactive hence continuing with deletion flow");
				} else {
