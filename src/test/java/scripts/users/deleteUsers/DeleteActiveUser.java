package scripts.users.deleteUsers;
...
public class DeleteActiveUser extends TestBase {
	UserCredentials userCredentials = null;
	PageObjectManager pageObjectManager = null;
	WebDriver driver = null;
	Wait<WebDriver> wait = null;

	public DeleteActiveUser() {

	}

	public DeleteActiveUser(PageObjectManager pageObjectManager, WebDriver driver, Wait<WebDriver> wait,
	...
	public void initializeTest(@Optional("yes") String excelFileStatus, @Optional("chrome") String browser,
		...
		testResultsStatus.setTestName(DeleteActiveUser.class.getSimpleName());
	}
	...
	public void startTest() {
		...
		logicForDeleteActiveUser(pageObjectManager.getUserCredentials().userToBeDeleted);
	}

	public TestResultsStatus logicForDeleteActiveUser(String userEmail) {
		...
		try {
			...
			// Check user status
			try {
				log.info("User email considered for deleting is : " + userEmail);
				if (pageObjectManager.getUsersPage().VerifyUserStatus(userEmail).equals("Active")) {
					log.info("User status is Active hence continuing with deletion flow");
				} else {
				...
				String errorMessage = "Error while getting user status\n";
			...
			try {
				pageObjectManager.getUsersPage().navigateToDeleteIconForUser(userEmail);
			} catch (Exception e) {
				e.printStackTrace();
				testResultsStatus.setTestResult("Fail");
				testResultsStatus.setTestResultComment(e.getMessage());
				log.error(e.getMessage());
				log.error("error while navigating to delete button");
				Assert.assertTrue(false);
			}

			// Delete user
			try {
				pageObjectManager.getUsersPage().deleteUser(userEmail);
			} catch (Exception e) {
				e.printStackTrace();
				testResultsStatus.setTestResult("Fail");
				testResultsStatus.setTestResultComment(e.getMessage());
			...
			try {
				Assert.assertEquals(message, "The user " + userEmail + " has been deleted");
		...
		pageObjectManager.getActivityLogsPage().AssertActivityLogs(
				getMessage("activityLogs.UserDeleted").replace("{companyName}", userCredentials.companyName)
