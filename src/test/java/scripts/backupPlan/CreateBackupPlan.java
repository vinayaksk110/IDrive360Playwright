...
public class CreateBackupPlan extends TestBase {
	...
	public CreateBackupPlan(PageObjectManager pageObjectManager, WebDriver driver, Wait<WebDriver> wait,
	...
	public void initializeTest(@Optional("yes") String excelFileStatus, @Optional("chrome") String browser,
		...
		testResultsStatus.setTestName(CreateBackupPlan.class.getSimpleName());
	}
	...
	public void startTest() {
		...
		logicForCreateBackupPlan();

	}

	public TestResultsStatus logicForCreateBackupPlan() {
		String compName = pageObjectManager.getLhsOptionsPage().getCompanyName();
		log.info("Company name is: " + compName);
		pageObjectManager.getBackupPlanPage().clickCreatePlan(testResultsStatus);
		pageObjectManager.getBackupPlanPage().enterDataAndCreateBackupPlan(userCredentials, testResultsStatus);
		String successMessage = pageObjectManager.getBackupPlanPage().waitForSuccessMessage();
		String ExpectedMessage = getMessage("backupplans.createdSuccMessage").replace("{}", compName);
		try {
		...
		pageObjectManager.getActivityLogsPage().AssertActivityLogs(
				getMessage("activitylogs.BackupPlanCreated").replace("{}", compName), testResultsStatus);
