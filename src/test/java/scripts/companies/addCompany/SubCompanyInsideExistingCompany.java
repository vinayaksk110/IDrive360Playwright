...
public class SubCompanyInsideExistingCompany extends TestBase {
	UserCredentials userCredentials = null;
	PageObjectManager pageObjectManager = null;
	WebDriver driver = null;
	Wait<WebDriver> wait = null;

	public SubCompanyInsideExistingCompany() {

	}

	public SubCompanyInsideExistingCompany(PageObjectManager pageObjectManager, WebDriver driver, Wait<WebDriver> wait,
	...
	public void initializeTest(@Optional("yes") String excelFileStatus, @Optional("chrome") String browser,
		...
		testResultsStatus.setTestName(SubCompanyInsideExistingCompany.class.getSimpleName());
	...
	public void AddUnit() {
		...
		try {
			userCredentials = localExcelUtility.getCompleteSheetData(getWorkbookPath_AccountsForTesting(), "Companies",
					"SubCompanyInsideExistingCompany");
		...
		logicForSubUnitInsideExistingUnit();

	}

	public TestResultsStatus logicForSubUnitInsideExistingUnit() {
		...
		try {
			...
			try {
				pageObjectManager.getCompaniesPage()
						.clickOnExistingCompany(pageObjectManager.getUserCredentials().parentCompanyName);
			} catch (Exception e) {
				String errorMessage = "Error while clicking on exisitng Company\n";
