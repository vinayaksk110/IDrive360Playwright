...
public class AddSubCompanyBelowExistingCompany extends TestBase {
	UserCredentials userCredentials = null;
	PageObjectManager pageObjectManager = null;
	WebDriver driver = null;
	Wait<WebDriver> wait = null;

	public AddSubCompanyBelowExistingCompany() {

	}

	public AddSubCompanyBelowExistingCompany(WebDriver driver, Wait<WebDriver> wait,
			PageObjectManager pageObjectManager, idrive360.utilities.TestResultsStatus testResultsStatus) {
		this.driver = driver;
		this.wait = wait;
	...
	public void initializeTest(@Optional("yes") String excelFileStatus, @Optional("chrome") String browser,
		...
		testResultsStatus.setTestName(AddSubCompanyBelowExistingCompany.class.getSimpleName());
	}
	...
	public void startTest() {
		...
		try {
			userCredentials = localExcelUtility.getCompleteSheetData(getWorkbookPath_AccountsForTesting(), "Companies",
					"AddSubCompanyBelowExistingCompany");
		...
		logicForAddSubCompanyBelowExistingCompany();

	}

	public TestResultsStatus logicForAddSubCompanyBelowExistingCompany() {
		...
		try {
			...
			// Click on add unit button from bottom
			try {
				pageObjectManager.getCompaniesPage().waitForBottomAddCompanyButtonAndClick();
			} catch (Exception e) {
				String errorMessage = "Error in the wait for bottom add Companies button\n";
