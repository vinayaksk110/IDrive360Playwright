...
public class CompanyHoverOverUserlistIconWithUnitHasNoUser extends TestBase {
	UserCredentials userCredentials = null;
	PageObjectManager pageObjectManager = null;
	WebDriver driver = null;
	Wait<WebDriver> wait = null;

	public CompanyHoverOverUserlistIconWithUnitHasNoUser() {

	}

	public CompanyHoverOverUserlistIconWithUnitHasNoUser(PageObjectManager pageObjectManager, WebDriver driver,
	...
	public void initializeTest(@Optional("yes") String excelFileStatus, @Optional("chrome") String browser,
		...
		testResultsStatus.setTestName(CompanyHoverOverUserlistIconWithUnitHasNoUser.class.getSimpleName());
	}
	...
	public void hoveroverUnitOption() {
		...
		try {
			userCredentials = localExcelUtility.getCompleteSheetData(getWorkbookPath_AccountsForTesting(), "Companies",
					"CompanyHoverOverUserlistIconWithUnitHasNoUser");
		...
		logicForCompanyHoverOverUserlistIconWithUnitHasNoUser();

	}

	public TestResultsStatus logicForCompanyHoverOverUserlistIconWithUnitHasNoUser() {
		try {
			...
			pageObjectManager.getCompaniesPage().userListIconClick(userCredentials.companyName);
			System.out.println("Clicked User list icon");
			// wait- LHS option-User is visible
			wait.until(ExpectedConditions.visibilityOf(pageObjectManager.getUsersPage().lhsUSerOption));
			// wait- Add user button in the center is visible with No User list
			wait.until(ExpectedConditions.visibilityOf(pageObjectManager.getUsersPage().centreAddUser));
		} catch (Exception e) {
