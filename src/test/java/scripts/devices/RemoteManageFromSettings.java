...
public class RemoteManageFromSettings extends TestBase {
	UserCredentials userCredentials = null;
	PageObjectManager pageObjectManager = null;
	WebDriver driver = null;
	Wait<WebDriver> wait = null;
	
	public RemoteManageFromSettings() {
		
	}
	
	public RemoteManageFromSettings(PageObjectManager pageObjectManager, WebDriver driver, Wait<WebDriver> wait,
	...
	public void initializeTest(@Optional("yes") String excelFileStatus, @Optional("chrome") String browser,
		...
		testResultsStatus.setTestName(RemoteManageFromSettings.class.getSimpleName());
	}
	
@Test
public void RemoteManageComputerFromSettings() {
	...
	try {
		...
		wait.until(ExpectedConditions.visibilityOf(pageObjectManager.getComputerDevicePage().btnComputerRemoteManage));
		action.moveToElement(pageObjectManager.getComputerDevicePage().btnComputerRemoteManage).perform();
		action.click(pageObjectManager.getComputerDevicePage().btnComputerRemoteManage).build().perform();
		System.out.println("remote manage clicked");
		WebElement BackupNow =driver.findElement(By.id("defBackup"));
		wait.until(ExpectedConditions.visibilityOf(BackupNow));
		System.out.println("Remote Manage opened successfully");
		testResultsStatus.setTestResultComment("Remote Manage opened successfully");
