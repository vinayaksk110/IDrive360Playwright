...
public class RemoveBackupAgentFromSettings extends TestBase {
	UserCredentials userCredentials = null;
	PageObjectManager pageObjectManager = null;
	WebDriver driver = null;
	Wait<WebDriver> wait = null;
	
	public RemoveBackupAgentFromSettings() {
		
	}
	
	public RemoveBackupAgentFromSettings(PageObjectManager pageObjectManager, WebDriver driver, Wait<WebDriver> wait,
	...
	public void initializeTest(@Optional("yes") String excelFileStatus, @Optional("chrome") String browser,
			@Optional("no") String headlessMode, @Optional("testserver25") String executionEnvironment)
				...
				testResultsStatus.setTestName(RemoveBackupAgentFromSettings.class.getSimpleName());
			}

	@Test
	public void RemoveBackupAgentofComputerFromSettings() {
		...
		try {
			...
			wait.until(ExpectedConditions.visibilityOf(pageObjectManager.getComputerDevicePage().btnRemoveBackupAgent));
			action.moveToElement(pageObjectManager.getComputerDevicePage().btnRemoveBackupAgent).perform();
			action.click(pageObjectManager.getComputerDevicePage().btnRemoveBackupAgent).build().perform();
			System.out.println("remove backup agent clicked");
			wait.until(ExpectedConditions.visibilityOf(pageObjectManager.getComputerDevicePage().btnremove));
			pageObjectManager.getComputerDevicePage().btnremove.click();
			System.out.println("remove button clicked");
			System.out.println("Backup agent from selected online device(s) has been removed.");
			testResultsStatus.setTestResultComment("Backup agent from selected online device(s) has been removed.");
